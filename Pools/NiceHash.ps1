...
$NiceHash_Request.result.simplemultialgo | ForEach-Object {
    ...
    if ([Double]$_.paying -eq 0.0) {
        Write-Log "Pool API ($Name, $NiceHash_Algorithm) returned price of zero. "
        return
    }

    $Divisor = 1000000000

    $Stat = Set-Stat -Name "$($Name)_$($NiceHash_Algorithm_Norm)_Profit" -Value ([Double]$_.paying / $Divisor *0.95) -Duration $StatSpan -ChangeDetection $true
